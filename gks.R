...
getTableFromHtm <- function(ref) {
    ...
    con <- file(tmp_filename_res, encoding = "CP1251")
    ...
    # parse as html
    html_nodes <- htmlTreeParse(html_raw, encoding = 'CP1251')
...
loadGKSData <- function(ref){
    ...
    switch(ext,
           doc = {
               ...
               doc.flnm <- gsub('.*[/]', '', ref)
               download(ref, doc.flnm, mode = "wb")
               # system(paste0(getwd(), '/SaveWordDocAsDocx.ps1'))
